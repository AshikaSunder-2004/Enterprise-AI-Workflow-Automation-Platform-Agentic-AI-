...
function validateNode(node) {
    ...
    switch (node.type) {
        case 'agent':
            ...
            if (!node.allowedTools?.length)
                errors.push(`${prefix}: allowedTools cannot be empty`);
            if (!node.outputKey)
                errors.push(`${prefix}: outputKey is required`);
        ...
        case 'branch':
            if (!node.conditions?.length)
                errors.push(`${prefix}: at least one condition is required`);
            if (!node.defaultTargetNodeId)
                errors.push(`${prefix}: defaultTargetNodeId is required`);
            break;
        case 'loop':
            if (!node.exitCondition)
                errors.push(`${prefix}: exitCondition is required`);
            if (node.maxIterations < 1)
                errors.push(`${prefix}: maxIterations must be >= 1`);
            break;
        case 'subworkflow':
            if (!node.workflowId)
                errors.push(`${prefix}: workflowId is required`);
            if (!node.outputKey)
                errors.push(`${prefix}: outputKey is required`);
            break;
    }
