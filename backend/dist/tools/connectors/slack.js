...
exports.slackReadChannelTool = exports.slackSendMessageTool = void 0;
...
exports.slackSendMessageTool = {
    ...
    async execute(input, ctx) {
        ...
        const token = ctx.connectorConfig?.slack_bot_token ?? process.env.SLACK_BOT_TOKEN;
        if (!token) {
        ...
        try {
            const response = await axios_1.default.post('https://slack.com/api/chat.postMessage', { channel, text, blocks }, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' } });
            if (!response.data.ok) {
...
exports.slackReadChannelTool = {
    ...
    async execute(input, ctx) {
        ...
        try {
            const response = await axios_1.default.get('https://slack.com/api/conversations.history', {
