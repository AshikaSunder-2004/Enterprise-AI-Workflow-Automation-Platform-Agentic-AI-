...
export class GuardrailsEngine {
  ...
  check(
    ...
    if (!allowedTools.includes(toolCall.name)) {
      logger.warn('Policy violation: disallowed tool', { tool: toolCall.name, allowed: allowedTools });
      return {
    ...
    if (budgetTokens && tokensUsed >= budgetTokens) {
      logger.warn('Policy violation: budget exceeded', { tokensUsed, budgetTokens });
      return {
    ...
    for (const { pattern, reason } of BLOCKED_ARG_PATTERNS) {
      if (pattern.test(argsStr)) {
        logger.warn('Policy violation: blocked argument pattern', { tool: toolCall.name, reason });
        return {
    ...
    if (HIGH_RISK_TOOLS.has(toolCall.name)) {
      logger.info('High-risk tool call allowed', { tool: toolCall.name });
    }
...
export const guardrails = new GuardrailsEngine();
